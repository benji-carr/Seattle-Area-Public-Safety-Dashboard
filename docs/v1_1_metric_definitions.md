...
| Current selected period | Inclusive calendar dates `[S, E]`. Crime state contains `start_date`, `end_date`, `crime_categories`, `crime_subcategories`, `neighborhoods`. The crime dashboard defaults to the latest one-week interval anchored to the latest valid crime event date: `[latest - 7 days, latest]`. Analysis bounds remain inclusive of both endpoints. |
...
**Crime map:** `prepare_crime_choropleth_data()` counts unique analytical `offense_id` from `valid_time`, including offenses without usable coordinates. Polygon counts reconcile to recognized-MCPP offenses; unassigned offenses remain in the analytical summary. Raw counts are the default. Rate mode shows selected-period rates **per 100,000**, without annualization; comparative shading requires estimated population of at least 5,000. All boundary polygons remain rendered in neighborhood/both modes. Disabled regions fade and do not define the color domain. Ctrl/Command-click updates the existing global Neighborhood dropdown (`[]` means all enabled); the last enabled region cannot be disabled by this interaction.

`prepare_crime_point_source()` copies the coordinate-valid `event_mcpp` population and joins authoritative analytical geography from `valid_time` for global neighborhood filtering, with no local fallback. Point text search affects only points. Neighborhoods, Points, and Both select rendered layers; the visible-point label counts rendered point traces and is zero in Neighborhoods mode. Inline and custom fullscreen views share metric, layer, filters, and colorbar settings. Both use MapLibre and Viridis; fullscreen daily-chart behavior remains independent.
...
- `tests/dashboard/test_crime_map.py::test_raw_counts_reconcile_unique_analytical_offenses_including_nonpoints` — verifies raw polygon counts and assigned/unassigned reconciliation use analytical offenses, including nonpoints and multiple offenses sharing a report.
- `tests/dashboard/test_crime_map.py::test_points_use_analytical_assignment_without_mutating_or_falling_back` — verifies point neighborhood filtering uses shared analytical assignments without modifying the spatial source.
- `tests/dashboard/test_crime_map.py::test_rate_per_100k_threshold_and_active_color_domain` — verifies the per-100,000 formula, inclusive 5,000-person cutoff, and eligible/enabled color domain.
- `tests/dashboard/test_crime_map.py::test_disabled_regions_remain_clickable_with_metrics_and_rescaled_domain` — verifies disabled polygons retain metrics and interaction data while fading.
- `tests/dashboard/test_crime_map.py::test_layer_modes_maplibre_camera_and_hover` and `test_text_filter_changes_only_points` — verify MapLibre layers, camera revision, Viridis, concise hover content, and point-only search.
- `tests/dashboard/test_crime_map_interaction.py::test_toggle_updates_real_dropdown_in_option_order` and `test_toggle_rejects_invalid_payload_and_last_disabled_region` — verify global neighborhood state ownership, ordering, all-enabled collapse, and last-enabled protection.
- `tests/dashboard/test_crime_map_interaction.py::test_fullscreen_equals_inline_real_map_and_rendered_point_count` — verifies identical inline/fullscreen map state, rendered point counts, close/reopen, and fullscreen neighborhood synchronization.
- `tests/dashboard/test_crime_map_interaction.py::test_native_bridge_modifiers_both_graphs_repeated_events_and_daily_ignored` — exercises the JavaScript bridge for both graphs, Ctrl/meta modifiers, repeat events, binding retries, and ignored point/daily traces. Existing `test_crime_fullscreen.py` coverage continues protecting map/daily expand and close behavior.
